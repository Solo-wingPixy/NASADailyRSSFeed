...
public class NasaDailyDAO {
	...
	public static void Update(SQLiteDatabase db){
		...
		values.put(NasaDailyOpenHelper.IMAGE,
				file.getAbsolutePath());
		
		db.update(NasaDailyOpenHelper.TABLE_NAME_1,
	...
	public static Cursor Query(SQLiteDatabase db){
		return db.query(true, NasaDailyOpenHelper.TABLE_NAME_1, 
				result_columns, where, whereArgs, groupBy, having, orderBy, null, null);
	}
	
	public static boolean initDatabase(SQLiteDatabase db){
		ContentValues values=new ContentValues();
		values.put(NasaDailyOpenHelper.TITLE, "美国夫妻散步挖出千枚金币 价值千万美元(图)");
		values.put(NasaDailyOpenHelper.DATE, "2014年02月26日13:58");
		values.put(NasaDailyOpenHelper.IMAGE, "R.drawable.coin");
		values.put(NasaDailyOpenHelper.DESCRIPTION, 
		...
		db.insert(NasaDailyOpenHelper.TABLE_NAME_1, null, values);
		
		values.clear();
		values.put(NasaDailyOpenHelper.TITLE, "乌外交部：新政府组建后将立即恢复同俄对话");
		values.put(NasaDailyOpenHelper.DATE, "2014年02月26日13:47");
		values.put(NasaDailyOpenHelper.IMAGE, "R.drawable.whatever");
		values.put(NasaDailyOpenHelper.DESCRIPTION, 
